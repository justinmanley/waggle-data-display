...
Elm.EnvSense.Generate = Elm.EnvSense.Generate || {};
Elm.EnvSense.Generate.make = function (_elm) {
   "use strict";
   _elm.EnvSense = _elm.EnvSense || {};
   _elm.EnvSense.Generate = _elm.EnvSense.Generate || {};
   if (_elm.EnvSense.Generate.values)
   return _elm.EnvSense.Generate.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "EnvSense.Generate",
   $Basics = Elm.Basics.make(_elm),
   $Dict = Elm.Dict.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Random = Elm.Random.make(_elm),
   $Sensor = Elm.Sensor.make(_elm),
   $Stateful = Elm.Stateful.make(_elm),
   $Time = Elm.Time.make(_elm);
   var sensor = F2(function (sensorId,
   valueGenerators) {
      return A2($Stateful.andThen,
      $Stateful.sequence(valueGenerators),
      function (values) {
         return $Stateful.$return({_: {}
                                  ,data: values
                                  ,id: sensorId});
      });
   });
   var intify = function (v) {
      return _U.replace([["value"
                         ,$Basics.toFloat($Basics.floor(v.value))]],
      v);
   };
   var simple = F3(function (physicalQuantity,
   units,
   _v0) {
      return function () {
         switch (_v0.ctor)
                 ...
                 return function () {
                    return function () {
                       ...
                       return function () {
                          ...
                          switch (_v6.ctor)
                          {case "Just":
                             return $Stateful.$return(_U.insert("value",
                               _v6._0,
                               reading));
                             case "Nothing":
                             return function () {
         ...
         _U.badCase($moduleName,
         "between lines 80 and 91");
      }();
   });
   var fahrenheit = A3(simple,
   "Temperature",
   "&deg;F",
   {ctor: "_Tuple2"
   ,_0: 0
   ,_1: 200});
   var celsius$ = function (suffix) {
      return A3(simple,
      A2($Basics._op["++"],
      "Temperature",
      suffix),
   ...
   var celsius = celsius$("");
   var infraRedCamera = function () {
      var cs = A2($List.map,
      $Basics.toString,
      _L.range(1,4));
      var mkCoord = function (coord) {
         return A2($List.map,
         F2(function (x,y) {
            return A2($Basics._op["++"],
            x,
      ...
      return $List.map(celsius$)(A2($List.concatMap,
      mkCoord,
      cs));
   }();
   var humidity = A3(simple,
   "Humidity",
   "%RH",
   {ctor: "_Tuple2"
   ,_0: 0
   ,_1: 100});
   var luminousIntensity = $Stateful.map(intify)(A3(simple,
   "Luminous Intensity",
   " raw A/D",
   {ctor: "_Tuple2"
   ,_0: 0
   ,_1: 1024}));
   var acousticIntensity = $Stateful.map(intify)(A3(simple,
   "Acoustic Intensity",
   " raw A/D",
   {ctor: "_Tuple2"
   ,_0: 0
   ,_1: 1024}));
   var pressure = A3(simple,
   "Pressure",
   "PA",
   {ctor: "_Tuple2"
   ,_0: 999120
   ,_1: 1000000});
   var acceleration = function (dimension) {
      return A3(simple,
      A2($Basics._op["++"],
      "Acceleration",
      dimension),
   ...
   var vibration = A3(simple,
   "RMS Vibration",
   "g",
   {ctor: "_Tuple2",_0: 0,_1: 10});
   var magneticField = function (dimension) {
      return A3(simple,
      A2($Basics._op["++"],
      "Magnetic Field",
      dimension),
   ...
   var rawTemperature = $Stateful.map(intify)(A3(simple,
   "Temperature",
   " raw A/D",
   {ctor: "_Tuple2"
   ,_0: 0
   ,_1: 1024}));
   var sensors = _L.fromArray([A2(sensor,
                              "MLX90614ESF-DAA.Melexis.008-2013",
                              _L.fromArray([fahrenheit]))
                              ,A2(sensor,
                              "TMP421.Texas_Instruments.2012",
                              _L.fromArray([celsius]))
                              ,A2(sensor,
                              "BMP180.Bosch.2_5-2013",
                              _L.fromArray([pressure
                                           ,celsius]))
                              ,A2(sensor,
                              "MMA8452Q.Freescale.8_1-2013",
                              _L.fromArray([acceleration("X")
                                           ,acceleration("Y")
                                           ,acceleration("Z")
                                           ,vibration]))
                              ,A2(sensor,
                              "Thermistor_NTC_PR103J2.US_Sensor.2003",
                              _L.fromArray([rawTemperature]))
                              ,A2(sensor,
                              ...
                              _L.fromArray([luminousIntensity]))
                              ,A2(sensor,
                              "PDV_P8104.API.2006",
                              _L.fromArray([luminousIntensity]))
                              ,A2(sensor,
                              "HIH4030.Honeywell.2008",
                              _L.fromArray([humidity]))
                              ,A2(sensor,
                              ...
                              ,A2(sensor,
                              "MAX4466.Maxim.2001",
                              _L.fromArray([acousticIntensity]))
                              ,sensor("D6T-44L-06.Omron.2012")(A2($List._op["::"],
                              celsius$("PTAT"),
                              infraRedCamera))
                              ,A2(sensor,
                              "HMC5883.Honeywell.2013",
                              _L.fromArray([magneticField("X")
                                           ,magneticField("Y")
                                           ,magneticField("Z")]))
                              ,A2(sensor,
                              ...
                              _L.fromArray([humidity
                                           ,celsius]))]);
   var generate = function (now) {
      return function () {
         var seed = $Random.initialSeed($Basics.floor(now));
         var state = {_: {}
                     ,cache: $Dict.empty
                     ,seed: seed};
         return $Basics.fst(A2($Stateful.run,
         $Stateful.sequence(sensors),
         state));
      }();
   };
   var GenerateState = F2(function (a,
   b) {
      return {_: {}
             ,cache: a
             ,seed: b};
   });
   _elm.EnvSense.Generate.values = {_op: _op
                                   ,generate: generate};
   return _elm.EnvSense.Generate.values;
};
Elm.EnvSense = Elm.EnvSense || {};
Elm.EnvSense.View = Elm.EnvSense.View || {};
Elm.EnvSense.View.make = function (_elm) {
   "use strict";
   _elm.EnvSense = _elm.EnvSense || {};
   _elm.EnvSense.View = _elm.EnvSense.View || {};
   if (_elm.EnvSense.View.values)
   return _elm.EnvSense.View.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "EnvSense.View",
   $Basics = Elm.Basics.make(_elm),
   ...
   $View$Util = Elm.View.Util.make(_elm);
   var infraRedCamera = function (history) {
      return function () {
         var initialReading = function (seed) {
            return {ctor: "_Tuple2"
                   ,_0: 0
                   ,_1: _U.replace([["value",0]],
                   seed)};
         };
         var sumReadings = F2(function (v1,
         _v0) {
            return function () {
               switch (_v0.ctor)
               {case "_Tuple2":
            ...
            return function () {
               ...
               {case "_Tuple2":
                  return _U.replace([["value"
                    ...
                    _v4._1);}
               _U.badCase($moduleName,
               "on line 152, column 44 to 81");
            }();
         };
         var casing = "TemperaturePTAT";
         var averageTemperature = $QueueBuffer.fromList($Config.historySize)($List.map(average)(A3($Util.groupFold,
         byTimestamp,
         sumReadings,
         initialReading)($List.concatMap($QueueBuffer.toList)($Dict.values(A2($Dict.remove,
         casing,
         history))))));
   ...
   var side = function (sensorId) {
      return function () {
         ...
         {case "BMP180.Bosch.2_5-2013":
            return $View$Util.Right;
            case "D6T-44L-06.Omron.2012":
            return $View$Util.Left;
   ...
   var viewSensor = function (_v10) {
      return function () {
         switch (_v10.ctor)
         {case "_Tuple2":
            return function () {
                 var viewOrdinary = function ($) {
                    return $Graphics$Element.flow($Graphics$Element.down)($List.map($View$Util.viewReadingHistory)($Dict.toList($)));
                 };
                 return $View$Util.sensorContainer(name(_v10._0))(function () {
                    var _v14 = name(_v10._0);
                    switch (_v14)
                    {case "D6T44L06":
                       return function ($) {
                            return viewOrdinary(infraRedCamera($));
                         }(_v10._1);
                       case "HMC5883":
                       return function ($) {
                            return viewOrdinary(magneticField(_v10._0)($));
                         }(_v10._1);
                       case "MMA8452Q":
                       return function ($) {
                            return viewOrdinary(acceleration(_v10._0)($));
                         }(_v10._1);}
                    return viewOrdinary(_v10._1);
                 }());
              }();}
         _U.badCase($moduleName,
   ...
   _elm.EnvSense.View.values = {_op: _op
...
Elm.Main.make = function (_elm) {
   ...
   $moduleName = "Main",
   $Basics = Elm.Basics.make(_elm),
   $EnvSense$Generate = Elm.EnvSense.Generate.make(_elm),
   $Graphics$Element = Elm.Graphics.Element.make(_elm),
   $Sensor = Elm.Sensor.make(_elm),
   ...
   var sensorData = $Update.update($Time.timestamp(A2($Signal.map,
   $EnvSense$Generate.generate,
...
Elm.QueueBuffer.make = function (_elm) {
   ...
   $Maybe.Nothing);
   var length = function ($) {
      return $Queue.length(function (_) {
         return _.queue;
      }($));
   };
   var maxLength = function (buf) {
      return buf.available + length(buf);
   };
...
Elm.Sensor = Elm.Sensor || {};
Elm.Sensor.make = function (_elm) {
   "use strict";
   _elm.Sensor = _elm.Sensor || {};
   if (_elm.Sensor.values)
   return _elm.Sensor.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   $QueueBuffer = Elm.QueueBuffer.make(_elm),
   $Time = Elm.Time.make(_elm);
   var InternalReading = F3(function (a,
   b,
   c) {
      return _U.insert("units",
      b,
      _U.insert("value",a,c));
   });
   var InternalSensorSnapshot = F2(function (a,
   b) {
      return {_: {},data: b,id: a};
   });
   _elm.Sensor.values = {_op: _op
                        ,InternalSensorSnapshot: InternalSensorSnapshot
...
Elm.Stateful = Elm.Stateful || {};
Elm.Stateful.make = function (_elm) {
   "use strict";
   _elm.Stateful = _elm.Stateful || {};
   if (_elm.Stateful.values)
   return _elm.Stateful.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   $Basics = Elm.Basics.make(_elm),
   $List = Elm.List.make(_elm);
   var run = F2(function (stateful,
   seed) {
      return function () {
         switch (stateful.ctor)
         {case "Stateful":
            return stateful._0(seed);}
         _U.badCase($moduleName,
         "between lines 39 and 43");
      }();
   });
   var Stateful = function (a) {
      return {ctor: "Stateful"
             ,_0: a};
   };
   var $return = function (val) {
   ...
   var get = Stateful(function (state) {
      return {ctor: "_Tuple2"
             ,_0: state
             ,_1: state};
   });
   var modify = function (modifier) {
      return Stateful(function (oldState) {
         return {ctor: "_Tuple2"
                ,_0: {ctor: "_Tuple0"}
                ,_1: modifier(oldState)};
      });
   };
   var put = function (newState) {
      return modify($Basics.always(newState));
   };
   var map = F2(function (f,
   stateful) {
      return Stateful(function (s) {
         return function () {
            var $ = A2(run,stateful,s),
            a = $._0,
            s$ = $._1;
            return {ctor: "_Tuple2"
                   ,_0: f(a)
   ...
   var andThen = F2(function (stateful,
   f) {
      return Stateful(function (s) {
         return function () {
            var $ = A2(run,stateful,s),
   ...
   var sequence = function (statefuls) {
      return function () {
         switch (statefuls.ctor)
         {case "::": return A2(andMap,
              A2(map,
              F2(function (x,y) {
                 return A2($List._op["::"],
                 x,
              ...
              statefuls._0),
              sequence(statefuls._1));
            case "[]":
            return $return(_L.fromArray([]));}
         _U.badCase($moduleName,
         "between lines 122 and 125");
      }();
   };
   _elm.Stateful.values = {_op: _op
                          ,$return: $return
                          ,run: run
                          ,get: get
                          ,put: put
                          ...
                          return function () {
                               ...
                               history) {
                                  return function () {
                                     ...
                                     _v0._0,
                                     _U.remove("physicalQuantity",
                                     value));
                                     var empty = $QueueBuffer.empty($Config.historySize);
...
Elm.View.make = function (_elm) {
   ...
   $Dict = Elm.Dict.make(_elm),
   $EnvSense$View = Elm.EnvSense.View.make(_elm),
   $Graphics$Element = Elm.Graphics.Element.make(_elm),
   $List = Elm.List.make(_elm),
   $Sensor = Elm.Sensor.make(_elm),
   $Time = Elm.Time.make(_elm),
   ...
   var datetime = function (time) {
      return $View$Util.h2($Date$Format.format("%B %d, %Y at %H:%M:%S")($Date.fromTime(time)));
   };
            ...
            return function () {
                 ...
                 {case "_Tuple2":
                    return function () {
                         ...
                         F2(function (sensorId,_v8) {
                            return function () {
                               return _U.eq($EnvSense$View.side(sensorId),
                               $View$Util.Left);
                            }();
                         }),
                         _v1._1),
                         ...
                         var dataDisplay = function ($) {
                            return center($Graphics$Element.flow($Graphics$Element.right)($List.map(centerVertically)($)));
                         }(_L.fromArray([$Graphics$Element.flow($Graphics$Element.down)($List.intersperse($View$Util.hline(function (_) {
                                           return _.width;
                                        }($Config.sensor)))($List.map($EnvSense$View.viewSensor)($List.sortWith(F2(function (s1,
                                        s2) {
                                           return A2($EnvSense$View.order,
                                           $Basics.fst(s1),
                                           $Basics.fst(s2));
                                        }))($Dict.toList(leftLayout)))))
                                        ,$View$Util.marginX(function (_) {
                                           return _.marginX;
                                        }($Config.image))(A3($Graphics$Element.image,
                                        function (_) {
                                           return _.width;
                                        }($Config.image),
                                        function (_) {
                                        ...
                                        $Config.sensorImageUrl))
                                        ,$Graphics$Element.flow($Graphics$Element.down)($List.intersperse($View$Util.hline(function (_) {
                                           return _.width;
                                        }($Config.sensor)))($List.map($EnvSense$View.viewSensor)($List.sortWith(F2(function (s1,
                                        s2) {
                                           return A2($EnvSense$View.order,
                         ...
                         return $Graphics$Element.layers(_L.fromArray([$View$Util.h1($Config.title)
...
Elm.View = Elm.View || {};
Elm.View.Chart = Elm.View.Chart || {};
Elm.View.Chart.make = function (_elm) {
   "use strict";
   _elm.View = _elm.View || {};
   _elm.View.Chart = _elm.View.Chart || {};
   if (_elm.View.Chart.values)
   return _elm.View.Chart.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "View.Chart",
   $Basics = Elm.Basics.make(_elm),
   $Color = Elm.Color.make(_elm),
   $Graphics$Collage = Elm.Graphics.Collage.make(_elm),
   $Graphics$Element = Elm.Graphics.Element.make(_elm),
                              ...
                              {case "::":
                                 switch (_v2._1._1.ctor)
                                   {case "::": return function () {
                                           ...
                                           var maxLength = $Basics.toFloat($QueueBuffer.maxLength(buf));
                                           var $ = {ctor: "_Tuple2"
                                                   ,_0: 2
                                                   ,_1: 2},
                                           marginX = $._0,
                                           ...
                                           var xOffset = w * ($Basics.toFloat(buf.available) / maxLength) / 2;
                                           ...
                                           var xScale = w * (size / maxLength) / (dataBbox.x.max - dataBbox.x.min);
   ...
   _elm.View.Chart.values = {_op: _op
                            ,chart: chart};
   return _elm.View.Chart.values;
};
Elm.View = Elm.View || {};
Elm.View.Util = Elm.View.Util || {};
Elm.View.Util.make = function (_elm) {
   "use strict";
   _elm.View = _elm.View || {};
   _elm.View.Util = _elm.View.Util || {};
   if (_elm.View.Util.values)
   return _elm.View.Util.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "View.Util",
   $Basics = Elm.Basics.make(_elm),
   $Color = Elm.Color.make(_elm),
   $Config = Elm.Config.make(_elm),
   $Graphics$Element = Elm.Graphics.Element.make(_elm),
   ...
   $Text = Elm.Text.make(_elm),
   $Util = Elm.Util.make(_elm),
   $View$Chart = Elm.View.Chart.make(_elm);
   ...
   var h2 = function ($) {
      return marginY(10)($Graphics$Element.leftAligned($Text.style($Config.h2Style)($Text.fromString($))));
   };
   var h1 = function ($) {
      return marginY(10)($Graphics$Element.leftAligned($Text.style($Config.h1Style)($Text.fromString($))));
   };
   var primaryText = function ($) {
      return $Text.style($Config.primaryStyle)($Text.fromString($));
   };
   ...
   v) {
      return function () {
         ...
         var units = primaryText(v.units);
         var name = function (el) {
            return A4($Graphics$Element.container,
            $Config.physicalQuantity.width,
            $Graphics$Element.heightOf(el),
            $Graphics$Element.midBottom,
            el);
         }(function ($) {
            return $Graphics$Element.leftAligned(primaryText($String.toLower($)));
         }(quantityName));
   ...
   var viewReadingHistory = function (_v0) {
      return function () {
         switch (_v0.ctor)
         {case "_Tuple2":
            return function () {
                 ...
                 $Graphics$Element.empty,
                 _v0._1);
                 var historyChart = $View$Chart.chart($Config.chart)(A2($QueueBuffer.map,
                 function (_v4) {
                    return function () {
                       return {ctor: "_Tuple2"
                              ,_0: _v4.timestamp
   ...
   _elm.View.Util.values = {_op: _op
                           ...
                           ,padding: padding
                           ,hline: hline
                           ,viewReadingHistory: viewReadingHistory
